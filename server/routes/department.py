...
def _has_consecutive_theory_conflict(assigned_slots, faculty_id, day_order, slot_index):
    """
    Return True if placing a theory class for faculty_id at (day_order, slot_index)
    would create two consecutive theory classes for that faculty on that day.
    Labs are never flagged — only theory→theory adjacency is forbidden.
    """
...
def generate_section_timetable(section, subjects, allocation_map):
    ...
    # ✅ RULE: theory = 2 times, lab = 1 time
    ...
    # ✅ NEW: prevent same subject in same day
    ...
    def candidate_slots(session):
        ...
        for day_order in range(len(TIMETABLE_DAYS)):
            for slot_index in range(len(TIMETABLE_SLOTS)):
                ...
                # ❗ NEW RULE: no same subject twice in same day
    ...
    #     subj_code = session['subject_code']
    #     incoming_theory = not session.get('is_lab', False)
    ...
    #             if slot_key in assigned_slots:
    #                 continue
    #             if fac_id in busy_slots.get(slot_key, set()):
    #                 continue
    #             # No consecutive theory classes for same faculty
    #             if incoming_theory and _has_consecutive_theory_conflict(assigned_slots, fac_id, day_order, slot_index):
    #                 continue

    #             ranking = (
    #                 day_load[day_order] >= max_day_load,
    #                 subject_day_usage[(subj_code, day_order)] > 0,
    #                 faculty_day_usage[(fac_id, day_order)],
    #                 day_load[day_order],
    #                 slot_index,
    #             )
    #             if subject_day_usage[(subj_code, day_order)] == 0:
    ...
    #         'Check that no faculty teaches too many subjects and that '
    #         'there are enough slots for all sessions.'
    #     )
...
def save_section_timetable(section, subjects, allocation_map, raw_entries):
    """
    Validate and save a manually submitted timetable.

    Checks:
    - Valid subject codes and allocations
    - No duplicate slots
    - Faculty conflict with other sections
    - No consecutive theory classes for same faculty
    - Correct count per subject
    """
    ...
    for item in raw_entries:
        ...
        # ── Faculty conflict across sections ─────────────────────────
